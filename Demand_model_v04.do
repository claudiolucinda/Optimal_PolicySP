...
est store mod01
estimates save "Data/modelo.ster", replace

adopath + "C:\Users\claudiolucinda\Dropbox\Lucinda-Moita\pedagio_otimo\Codigos_CRL\Code_Analysis\Optimal_PolicySP\"
cap program drop El_mixlogit
El_mixlogit if e(sample), price(costcost) choicevar(dup) priceint(costxrenda)

mat elasts=r(elast)

mat rownames elasts=Bus Rail Driving Motorcycle Taxi Other
mat colnames elasts=Bus Rail Driving Motorcycle Taxi Other

estout matrix(elasts, fmt(%9.4f)) using ".\Data\Elasticities.txt", style(tab) replace

qui estout mod01  using ".\Data\mixlogit.txt", cells(b(fmt(%6.4f) star) t(par fmt(%6.4f)) ) /// 
stats(N chi2 ll, labels(`"Observations"' `"LR chi2"' `"Log-Lik."')) label collabels(,none) style(tab) ///
stardetach order(*_1 *_2 *_3 *_4 *_5 timetime costcost timexrenda costxrenda SD:costcost SD:timetime) eqlabels(Fixed Random,span) replace
...
qui mixlpred pasclogit_0, nrep(50)
save "Data/dados_start00.dta", replace

*save "Data/Res_pt1.dta", replace
